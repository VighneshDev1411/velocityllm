...
const statusColors: Record<string, string> = { success: '#22c55e', failed: '#ef4444', running: '#4b8eff', pending: '#f59e0b', cancelled: '#4b5563', skipped: '#4b5563' };
...
function PipelineCard({ pipeline }: { pipeline: any }) {
  ...
  return (
    <Paper elevation={0} sx={{ p: 2.5, backgroundColor: '#141922', border: '1px solid #1e2736', borderRadius: '8px' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
        <Box>
          <Typography sx={{ fontSize: '0.9rem', fontFamily: MONO, color: '#e5e2e1', fontWeight: 700 }}>{pipeline.name}</Typography>
          <Box sx={{ display: 'flex', gap: 0.5, mt: 0.5 }}>
            <Chip label={pipeline.branch} size="small" icon={<GitBranch className="w-3 h-3" />} sx={{ fontFamily: MONO, fontSize: '0.6rem', height: 20, backgroundColor: '#1e2736', color: '#c1c6d7', '& .MuiChip-icon': { color: '#c1c6d7' } }} />
        ...
        <Button
          ...
          sx={{ fontFamily: MONO, fontSize: '0.65rem', textTransform: 'none', borderColor: '#1e2736', color: '#22c55e' }}
        >
            ...
            return (
              <>
                <TableRow key={run.id} sx={{ '&:hover': { backgroundColor: '#1a2332' }, cursor: 'pointer' }} onClick={() => setExpanded(expanded === run.id ? null : run.id)}>
                  ...
                  <TableCell sx={{ color: '#e5e2e1', fontFamily: MONO, fontSize: '0.75rem', borderColor: '#1e2736', fontWeight: 600 }}>{run.pipeline_name}</TableCell>
                  <TableCell sx={{ color: '#8b949e', fontFamily: MONO, fontSize: '0.7rem', borderColor: '#1e2736' }}>{run.branch}</TableCell>
                  <TableCell sx={{ borderColor: '#1e2736' }}>
                    <Box>
                  ...
                  <TableCell sx={{ color: '#8b949e', fontFamily: MONO, fontSize: '0.7rem', borderColor: '#1e2736' }}>{run.author}</TableCell>
                  <TableCell sx={{ color: '#e5e2e1', fontFamily: MONO, fontSize: '0.7rem', borderColor: '#1e2736' }}>{run.duration}</TableCell>
                  <TableCell sx={{ borderColor: '#1e2736' }}>
                    <Chip label={run.trigger} size="small" sx={{ fontFamily: MONO, fontSize: '0.55rem', height: 18, backgroundColor: '#1e2736', color: '#c1c6d7' }} />
                  </TableCell>
                  <TableCell sx={{ borderColor: '#1e2736' }}>
                ...
                {expanded === run.id && (
                  <TableRow>
                    <TableCell colSpan={8} sx={{ borderColor: '#1e2736', p: 0 }}>
                      <Box sx={{ p: 2, backgroundColor: '#0d1117' }}>
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          {(run.stages || []).map((stage: any, si: number) => (
                            <Box key={si} sx={{ flex: 1, p: 1.5, backgroundColor: '#141922', borderRadius: '6px', border: '1px solid #1e2736' }}>
        ...
        <TableBody>
          {(deployments || []).map((d: any) => (
            <TableRow key={d.id} sx={{ '&:hover': { backgroundColor: '#1a2332' } }}>
              <TableCell sx={{ color: '#4b8eff', fontFamily: MONO, fontSize: '0.8rem', borderColor: '#1e2736', fontWeight: 600 }}>{d.version}</TableCell>
              <TableCell sx={{ borderColor: '#1e2736' }}>
                <Chip label={d.environment} size="small" sx={{ fontFamily: MONO, fontSize: '0.6rem', height: 20, backgroundColor: d.environment === 'production' ? '#ef444420' : '#f59e0b20', color: d.environment === 'production' ? '#ef4444' : '#f59e0b' }} />
              </TableCell>
              <TableCell sx={{ borderColor: '#1e2736' }}>
              ...
              <TableCell sx={{ color: '#8b949e', fontFamily: MONO, fontSize: '0.75rem', borderColor: '#1e2736' }}>{d.deployed_by}</TableCell>
              <TableCell sx={{ color: '#8b949e', fontFamily: MONO, fontSize: '0.7rem', borderColor: '#1e2736' }}>{new Date(d.deployed_at).toLocaleString()}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
