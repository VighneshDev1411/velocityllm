...
import {
  BarChart2, Download, FileDown, Image as ImageIcon,
} from 'lucide-react';
...
function buildRadarData(models: any[]): { metric: string; [key: string]: any }[] {
  ...
  // For each metric, find min/max to normalize to 0–100
  ...
  return metrics.map(({ label, extract, invert }) => {
    ...
    topModels.forEach((m, idx) => {
      const raw = extract(m);
      let normalized = ((raw - min) / range) * 80 + 20; // scale to 20–100
      if (invert) normalized = 120 - normalized; // higher is better for inverted
...
function buildTreemapData(costBreakdown: any): any[] {
  ...
  // costBreakdown may be { models: [...] } or an array
...
function buildHeatmapData(timeSeries: any): number[][] {
  // 7 rows (days) x 24 cols (hours), init to 0
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        ...
        <MuiTooltip title="Export as PNG">
          <IconButton
            size="small"
            onClick={() => exportChartAsPNG(id, `velocityllm-${id}`)}
  ...
  return (
    <g>
      <rect
        ...
        height={height}
        rx={4}
        fill={PROVIDER_COLORS[provider?.toLowerCase()] || '#6b7280'}
        fillOpacity={0.85}
        stroke="#fff"
        strokeWidth={2}
      />
...
export default function VisualizationPage() {
  ...
  // Bar chart data — per-model latency, cost, throughput
  const barData = topModels.map((m: any) => ({
    name: (m.model_name || m.model || 'unknown').replace(/^.*\//, ''),
    latency: m.avg_latency_ms || 0,
    cost: parseFloat(m.total_cost?.toString().replace('$', '') || '0') * 100, // scale for visibility
  ...
  if (summaryError) {
    return (
      <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Alert severity="error" sx={{ maxWidth: 400, borderRadius: '12px' }}>
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 } }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        {/* ---------------------------------------------------------------- */}
        {/* 1. Radar Chart — Model Comparison                                */}
        {/* ---------------------------------------------------------------- */}
          ...
          {radarData.length === 0 ? (
            <Box sx={{ height: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Typography sx={{ color: 'text.disabled', fontSize: '0.875rem' }}>
                No model data available for radar chart
              </Typography>
            </Box>
          ) : (
            <Box sx={{ height: 360 }}>
        ...
        {/* ---------------------------------------------------------------- */}
        {/* 2. Treemap + 3. Bar Chart — side by side                         */}
        {/* ---------------------------------------------------------------- */}
                ...
                <Box sx={{ height: 300 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={barData} barGap={4} barCategoryGap="20%">
                      ...
                      <Bar dataKey="cost" name="Cost (x100)" fill="#10b981" radius={[4, 4, 0, 0]} />
        ...
        <ChartPanel id="viz-heatmap" title="Usage Heatmap (Day x Hour)" loading={tsLoading}>
          {heatmapData.flat().every((v) => v === 0) ? (
            <Box sx={{ height: 260, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Typography sx={{ color: 'text.disabled', fontSize: '0.875rem' }}>
                No time-series data available for heatmap
              </Typography>
            </Box>
          ) : (
            <>
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      backgroundColor: intensity === 0
                        ? '#f0f4f8'
                        : `rgb(${Math.round(219 - intensity * 160)},${Math.round(234 - intensity * 130)},${Math.round(254 - intensity * 50)})`,
                    }}
                  />
                ))}
